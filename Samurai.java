import java.awt.*;
import java.awt.event.KeyEvent;
import java.awt.image.BufferedImage;
import java.io.File;
import javax.imageio.ImageIO;

public class Samurai {
    private int x, y;
    private int width = 128, height = 128; // Scaled to sprite size
    private int speed = 8;

    // Combat State
    private int maxHealth = 100;
    private int currentHealth = 100;
    private boolean isDead = false;
    private boolean damageDealt = false;

    // Animation Frames
    private BufferedImage[] idleLeft, idleRight;
    private BufferedImage[] walkLeft, walkRight;
    private BufferedImage[] attack1Left, attack1Right;
    private BufferedImage[] attack2Left, attack2Right;
    private BufferedImage[] shieldLeft, shieldRight;
    private BufferedImage[] deadLeft, deadRight;

    // Current Animation State
    private BufferedImage[] currentFrames;
    private int currentFrameIndex = 0;
    private int animationTick = 0;
    private int animationSpeed = 5;

    // State
    private boolean isMoving = false;
    private boolean isAttacking = false;
    private boolean isShielding = false;
    private boolean facingLeft = true;

    public boolean isAttackingPublic() {
        return isAttacking;
    }

    public boolean isShieldingPublic() {
        return isShielding;
    }

    public boolean isDead() {
        return isDead;
    }

    public int getHealth() {
        return currentHealth;
    }

    public int getMaxHealth() {
        return maxHealth;
    }

    public Rectangle getBounds() {
        return new Rectangle(x, y, width, height);
    }

    public boolean hasDealtDamage() {
        return damageDealt;
    }

    public void setDamageDealt(boolean dealt) {
        this.damageDealt = dealt;
    }

    public Samurai(int startX, int startY) {
        this.x = startX;
        this.y = startY;
        loadResources();
        currentFrames = idleRight;
    }

    private void loadResources() {
        idleLeft = loadSpriteSheet("assets/characters/Samurai/A/Idle.png");
        walkLeft = loadSpriteSheet("assets/characters/Samurai/A/Walk.png");
        attack1Left = loadSpriteSheet("assets/characters/Samurai/A/Attack_1.png");
        attack2Left = loadSpriteSheet("assets/characters/Samurai/A/Attack_2.png");
        shieldLeft = loadSpriteSheet("assets/characters/Samurai/A/Shield.png");
        deadLeft = loadSpriteSheet("assets/characters/Samurai/A/Dead.png");

        idleRight = loadSpriteSheet("assets/characters/Samurai/D/Idle.png");
        walkRight = loadSpriteSheet("assets/characters/Samurai/D/Walk.png");
        attack1Right = loadSpriteSheet("assets/characters/Samurai/D/Attack_1.png");
        attack2Right = loadSpriteSheet("assets/characters/Samurai/D/Attack_2.png");
        shieldRight = loadSpriteSheet("assets/characters/Samurai/D/Shield.png");
        deadRight = loadSpriteSheet("assets/characters/Samurai/D/Dead.png");

        if (idleRight == null)
            System.out.println("Error: Samurai idleRight is null");
        if (walkRight == null)
            System.out.println("Error: Samurai walkRight is null");
        if (attack1Right == null)
            System.out.println("Error: Samurai attack1Right is null");
        if (attack2Right == null)
            System.out.println("Error: Samurai attack2Right is null");
        if (shieldRight == null)
            System.out.println("Error: Samurai shieldRight is null");
        if (deadRight == null)
            System.out.println("Error: Samurai deadRight is null");
    }

    private BufferedImage[] loadSpriteSheet(String path) {
        try {
            File f = new File(path);
            if (!f.exists()) {
                System.out.println("Missing asset: " + path);
                return null;
        }
            BufferedImage sheet = ImageIO.read(f);
            int h = sheet.getHeight();
            int w = sheet.getWidth();
            int count = w / h;
            System.out.println("Loaded " + path + ": " + count + " frames");
            BufferedImage[] frames = new BufferedImage[count];
            for (int i = 0; i < count; i++) {
                frames[i] = sheet.getSubimage(i * h, 0, h, h);
            }
            return frames;
        } catch (Exception e) {
            System.out.println("Error loading Samurai asset: " + path);
            e.printStackTrace();
            return null;
        }
    }

    public void takeDamage(int amount) {
        if (isDead || isShielding)
            return;

        currentHealth -= amount;

        if (currentHealth <= 0) {
            System.out.println("Samurai Died!");
            currentHealth = 0;
            isDead = true;
            currentFrameIndex = 0;


            System.out.println("Facing Left: " + facingLeft);
            if (facingLeft) {
                currentFrames = deadLeft;
                System.out.println("Selected deadLeft. Frames: " + (deadLeft != null ? deadLeft.length : "null"));
            } else {
                currentFrames = deadRight;
                System.out.println("Selected deadRight. Frames: " + (deadRight != null ? deadRight.length : "null"));
            }
        }
    }

    public void update() {
        // Animation Frame
        animationTick++;
        if (animationTick >= animationSpeed) {
            animationTick = 0;
            currentFrameIndex++;

            // Safety check
            if (currentFrames == null)
                return;

            if (currentFrameIndex >= currentFrames.length) {
                if (isDead) {
                    currentFrameIndex = Math.max(0, currentFrames.length - 1); // Stay dead
                } else if (isAttacking) {
                    isAttacking = false;
                    damageDealt = false;
                    currentFrameIndex = 0;
                    resetAnimationState();
                } else if (isShielding) {
                    currentFrameIndex = Math.max(0, currentFrames.length - 1);
                } else {
                    currentFrameIndex = 0;
                }
            }
        }
    }

    private void resetAnimationState() {
        if (isDead)
            return;
        if (!isAttacking && !isShielding) {
            if (isMoving) {
                currentFrames = facingLeft ? walkLeft : walkRight;
            } else {
                currentFrames = facingLeft ? idleLeft : idleRight;
            }
        }
    }

    public void setInput(int keyCode, boolean pressed) {
        if (isDead)
            return;

        // Movement
        if (keyCode == KeyEvent.VK_LEFT) {
            if (pressed) {
                isMoving = true;
                facingLeft = true;
                if (!isAttacking && !isShielding)
                    currentFrames = walkLeft;
            } else if (facingLeft) {
                isMoving = false;
                if (!isAttacking && !isShielding)
                    currentFrames = idleLeft;
            }
        }
        if (keyCode == KeyEvent.VK_RIGHT) {
            if (pressed) {
                isMoving = true;
                facingLeft = false;
                if (!isAttacking && !isShielding)
                    currentFrames = walkRight;
            } else if (!facingLeft) {
                isMoving = false;
                if (!isAttacking && !isShielding)
                    currentFrames = idleRight;
            }
        }

        // Actions
        boolean isActionKey = (keyCode == KeyEvent.VK_NUMPAD7 || keyCode == KeyEvent.VK_7 ||
                keyCode == KeyEvent.VK_NUMPAD8 || keyCode == KeyEvent.VK_8 ||
                keyCode == KeyEvent.VK_NUMPAD9 || keyCode == KeyEvent.VK_9);

        if (isActionKey) {
            if (pressed && !isAttacking && !isShielding) {
                if (keyCode == KeyEvent.VK_NUMPAD7 || keyCode == KeyEvent.VK_7) {
                    isAttacking = true;
                    damageDealt = false;
                    currentFrameIndex = 0;
                    currentFrames = facingLeft ? attack1Left : attack1Right;
                } else if (keyCode == KeyEvent.VK_NUMPAD8 || keyCode == KeyEvent.VK_8) {
                    isAttacking = true;
                    damageDealt = false;
                    currentFrameIndex = 0;
                    currentFrames = facingLeft ? attack2Left : attack2Right;
                } else if (keyCode == KeyEvent.VK_NUMPAD9 || keyCode == KeyEvent.VK_9) {
                    isShielding = true;
                    currentFrameIndex = 0;
                    currentFrames = facingLeft ? shieldLeft : shieldRight;
                }
            } else if (!pressed) {
                if (keyCode == KeyEvent.VK_NUMPAD9 || keyCode == KeyEvent.VK_9) {
                    isShielding = false;
                    resetAnimationState();
                }
            }
        }
    }

    public void physicsUpdate() {
        if (isDead || isAttacking || isShielding)
            return;

        if (isMoving) {
            int nextX = x + (facingLeft ? -speed : speed);
            int nextY = y;
            if (canMoveTo(nextX, nextY)) {
                x = nextX;
                y = nextY;
            }
        }
    }

    private boolean canMoveTo(int nextX, int nextY) {
        int hitboxSize = 64;
        int offsetX = (width - hitboxSize) / 2;
        int offsetY = (height - hitboxSize) / 2;
        int hx = nextX + offsetX;
        int hy = nextY + offsetY;

        // Collision logic is map-based only. Players don't collide with each other per
        // request.
        return isWalkable(hx, hy) && isWalkable(hx + hitboxSize, hy) &&
                isWalkable(hx, hy + hitboxSize) && isWalkable(hx + hitboxSize, hy + hitboxSize);
    }

    private boolean isWalkable(int px, int py) {
        if (px < 0 || px >= 800 || py < 0 || py >= 600)
            return false;
        try {
            return CollisionData.map[py][px] == 1;
        } catch (Exception e) {
            return false;
        }
    }

    public void draw(Graphics2D g2) {
        if (currentFrames != null && currentFrameIndex >= 0 && currentFrameIndex < currentFrames.length) {
            g2.drawImage(currentFrames[currentFrameIndex], x, y, width, height, null);
        } else {
            // Draw last frame if index overshoot? Or fallback rect
            if (currentFrames != null && currentFrames.length > 0) {
                g2.drawImage(currentFrames[Math.min(Math.max(0, currentFrameIndex), currentFrames.length - 1)], x, y,
                        width, height, null);
            } else {
                g2.setColor(Color.BLUE);
                g2.fillRect(x, y, width, height);
            }
        }
    }

    public int getX() {
        return x;
    }

    public int getY() {
        return y;
    }
}
